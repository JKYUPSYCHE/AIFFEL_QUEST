...
import joblib
from autoint.autointmlp import AutoIntMLPModel, predict_model
...
def load_data():
    ...
    ratings_df = pd.read_csv(data_path / movielens_dir_nm / 'ratings_prepro.csv')
    movies_df = pd.read_csv(data_path / movielens_dir_nm / 'movies_prepro.csv')
    user_df = pd.read_csv(data_path / movielens_dir_nm / 'users_prepro.csv')
    ...
    model.load_weights(model_path / 'autoIntMLP_model_weights.weights.h5')
    label_encoders = joblib.load(data_path / 'label_encoders.pkl')
